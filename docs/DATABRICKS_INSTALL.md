...
| 5.20.5 | — | On PyPI 2026-09-18, same mirror lag. No dependency change: 5.20.4 plus a wait-and-retry on the provider's rate limit (429) in `report_ingest/engine.py`. Expect the 5.20.0 log exactly. Superseded by 5.21.0. |
| 5.29.1 | — | **RELEASED to PyPI 2026-09-26 (tag `v5.29.1`) with planlens 0.10.1 — THIS IS THE VERSION TO INSTALL** (not 5.29.0), once the Nexus mirror delivers both a day or two later. **No dependency change**: 5.29.0 with `find_like` kept an optional tool (out of the prompts and hints); expect the 5.29.0 log with the version numbers changed (`planlens-0.10.1`); `anthropic` must NOT appear. Release gate: see HANDOFF §0a-current. Replace this row when the cluster install is confirmed. |
| 5.29.0 | — | **RELEASED to PyPI 2026-09-25 (tag `v5.29.0`) with planlens 0.10.0**, superseded by 5.29.1 the next day — install 5.29.1; once the Nexus mirror delivers both a day or two later. **No new package**: `planlens-0.10.0` in place of 0.9.0, dependencies identical (its new `find_like` uses the OpenCV planlens already required); expect the 5.26.0 log with the version numbers changed; `anthropic` must NOT appear. Carries `find_like` (every copy of a tag across a drawing set, verified by vision), robust-first vision (`GEOTECH_VISION_POLICY`, default `robust`: images at `original` wherever honoured — more image tokens per call, by the owner's choice), and tiled page reads for small lettering. Release gate: see HANDOFF §0a-current. Replace this row when the cluster install is confirmed. |
| 5.28.0 | — | **RELEASED to PyPI 2026-09-24 (tag `v5.28.0`) with planlens 0.9.0**, superseded by 5.29.0 the next day; once the Nexus mirror delivers both a day or two later. **No new package**: `planlens-0.9.0` in place of 0.8.0, whose dependencies are identical, so expect the 5.26.0 log with the version numbers changed; `anthropic` must NOT appear. Carries the vision probe (`funhouse_agent/vision_probe.py`: four small model calls once per process — the answering model's name and its real image budget, shown in Connection diagnostics as "vision model"; `GEOTECH_VISION_PROBE=0` switches it off), the legibility note, and the text-layer-first prompt rules. Release gate: see HANDOFF §0a-current. Replace this row when the cluster install is confirmed. |
| 5.27.0 | — | **RELEASED to PyPI 2026-09-23 (tag `v5.27.0`) with planlens 0.8.0**, superseded by 5.28.0 the next day; once the Nexus mirror delivers both a day or two later. **No new package**: the only move is `planlens-0.8.0` in place of 0.7.0, and planlens 0.8.0 declares **exactly** the dependencies 0.7.0 did (numpy, ezdxf, PyMuPDF, opencv-python-headless, rapidfuzz), so no other package should move on its account — expect the 5.26.0 log with the two version numbers changed. `anthropic` stays optional and must NOT appear. What the release carries: vision renders sized to the model (`funhouse_agent/vision_view.py`; env `GEOTECH_VISION_BUDGET`, `GEOTECH_CHART_BUDGET`, `GEOTECH_VISION_DETAIL`), chart read-offs at GPT-5.4's `original` detail, and `render_region(view=, image_box=)`. Release gate: see HANDOFF §0a-current. Replace this row when the cluster install is confirmed. |
| 5.26.0 | — | **RELEASED to PyPI 2026-09-22 (tag `v5.26.0`) with planlens 0.7.0**, superseded by 5.27.0 the next day; once the Nexus mirror delivers both a day or two later. **Two NEW packages and one pin move**, so the log will NOT be the 5.20.0 log: `python-docx` (pure Python; its `lxml` is already on the cluster via streamlit) and `markdown-it-py` (already there transitively through `rich`; now declared), and `planlens-0.7.0` in place of 0.6.0 — planlens 0.7.0 declares **exactly** the dependencies 0.6.0 did, so no other package should move on its account. `anthropic` stays optional and must NOT appear. What the release carries: the two-page app (`webapp/pages_entry.py`; `run_on_databricks` boots it by default, `pages=False` restores the single page), the document-review agent, per-page conversation folders (`<data root>/pages/document_review` beside the untouched geotech root), the Tiny Apps plumbing (dormant on Databricks), and the document-output tools `write_docx` and `annotate_document`. **If `python-docx` is quarantined by the Nexus firewall the install fails** — that is the one new risk; report the exact line. Release gate: see HANDOFF §0a-current. Replace this row when the cluster install is confirmed. |
| 5.25.0 | — | **RELEASED to PyPI 2026-09-21 (tag `v5.25.0`, master `46ae949`)**, superseded by 5.26.0 the next day; once the Nexus mirror delivers it a day or two later. **The report-ingest train is PARKED here**: pickup list in `HANDOFF.md` §0a-current. **No dependency change and no new package** — the pin stays `planlens>=0.6` and planlens itself does not move, because all five builds in this release are pure Python inside `report_ingest` over what is already on the cluster: the page-label vote and the disagreement-only review, bound reports as their own records, the calculation reader, the test-pit / CPT / DCP readers (whose DIGGS side is one new reader in `subsurface_characterization`, already installed), and the report library, whose full-text index is SQLite's own FTS5 from the standard library and whose fuzzy fallback is `rapidfuzz`, on the cluster with planlens since 0.4.0. So expect the 5.20.0 log exactly, with `geotech-staff-engineer-5.25.0`: `anthropic` stays optional and must NOT appear, and `openpyxl` still arrives with `python-ags4`. The wheel gains ONE data file, `report_ingest/library_questions.EXAMPLE.json` (synthetic, beside the `templates.json.EXAMPLE` that 5.24.0 added). It supersedes 5.22.1 and every unreleased row below. Release gate 13,108 passed / 33 skipped / 0 failed (3,598/28 + 8,086/5 + 1,424/0). Replace this row when the cluster install is confirmed. |
| 5.24.0 | — | On master 2026-09-20, **not yet tagged**. **No dependency change and no new package** — pure Python inside `report_ingest`: a log-TEMPLATE recogniser (`log_templates.py`) and the narrative reader's first round of accuracy levers. Both lean on `rapidfuzz`, which has arrived with `planlens` since 0.4.0 and is already on the cluster, so expect the 5.20.0 log exactly, with `geotech-staff-engineer-5.24.0`. The wheel gains one data file, `report_ingest/templates.json.EXAMPLE`. Install it once tagged; it supersedes 5.22.1. Replace this row when the cluster install is confirmed. |
| 5.23.0 | — | On master 2026-09-20, **not yet tagged**. **No dependency change and no new package** — pure Python inside `report_ingest`: the floor under the log and lab readers (the grid / the tables as the first voter; `floor.py`, `log_floor.py`, `lab_floor.py`) and the `ingest` stage (the whole graph per report on the cluster, DIGGS gated, scored against the hand truth). Expect the 5.20.0 log exactly, with `geotech-staff-engineer-5.23.0`. Install it once tagged; it supersedes 5.22.1. Replace this row when the cluster install is confirmed. |
| 5.22.1 | — | On PyPI 2026-09-21, same mirror lag; **install this one, never 5.22.0** (which does not import: a SyntaxError in `report_ingest/mirror.py` slipped past a piped test run). No dependency change. |
| 5.22.0 | — | On PyPI 2026-09-21 but **BROKEN** (does not import). Superseded by 5.22.1 the same hour. **No dependency change and no new package** — the whole release is pure Python inside `report_ingest`: a durable mirror of every run's output (`report_ingest/mirror.py`, standard library only — `json`, `os`, `shutil`, `pathlib`; the SharePoint side is the `fh_sp_client` object the notebook already holds and is never imported or constructed here) and a `vote` stage that scores the rules, the vision pass and the review against each other with **no model call**. Expect the 5.20.0 log exactly, with `geotech-staff-engineer-5.22.0`. **Install this one** once it is tagged; 5.20.1–5.21.2 are superseded. Replace this row when the cluster install is confirmed. |
| 5.21.2 | — | On PyPI 2026-09-20, same mirror lag; **no dependency change and no new package** — the whole release is pure Python inside `report_ingest`. It fixes the whole-report (`document`) vision mode after its first cluster run: page pictures travel as **JPEG** instead of PNG (the gateway refused every window of a scanned report as "the request entity is too large", which is a BODY limit the 50-image probe never reached), a window that is still refused halves itself and re-cuts the rest of the report, and a page no sheet or window answered for gets one page-mode call. JPEG encoding is **Pillow**, which is already there through `matplotlib>=3.8` and `streamlit>=1.39` — planlens and PyMuPDF declare no Pillow, so do not expect the pin to come from them. Expect the 5.20.0 log exactly, with `geotech-staff-engineer-5.21.2`. **Install this one**; 5.20.1–5.21.1 are superseded. Replace this row when the cluster install is confirmed. |
| 5.21.1 | — | On PyPI 2026-09-18, same mirror lag; no dependency change: 5.21.0 plus number-only contact sheets for the vision passes (planlens' sheets caption tiles with the page kind, which the model read as a label). **Install this one** when it arrives; everything from 5.20.1 to 5.21.0 is superseded. |
| 5.21.0 | — | On PyPI 2026-09-18 (superseded by 5.21.1 the same evening). **No dependency change and no new package of any kind** — the whole release is pure Python inside `report_ingest`: a third vision mode (`document`), OpenAI's `image_url.detail` on every picture, and the owner's own Funhouse deployment rates so every `## Cost` block prints dollars beside tokens. It draws the page-number stamp with **Pillow**, which is already there: `matplotlib>=3.8` and `streamlit>=1.39` both require it and both are core app dependencies — note that planlens and PyMuPDF declare NO Pillow, so do not expect the pin to come from them. Expect the 5.20.0 log exactly, with `geotech-staff-engineer-5.21.0`. **Install this one**; 5.20.1–5.20.5 are superseded. Replace this row when the cluster install is confirmed. |
