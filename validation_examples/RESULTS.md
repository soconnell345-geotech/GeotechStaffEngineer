...
| V-030 | slope_stability probabilistic reliability — Duncan (2000) LASH (Slide2 #29) | Taylor-series: F=1.17, COV_F≈0.16 → Pf 18.2% (lognormal & normal); per-method Pf bracketed | Duncan quote F=1.17, Pf 18% (Taylor); Slide2 per-method Pf 13-18% (F 1.127-1.168) | Pf +0.2% | **PASS** (reliability) / **N/A-scope** (FOSM-on-surface) | The module's Taylor-series reliability helpers (`lognormal_beta`/`normal_beta`/`pf`) reproduce Duncan's reported LASH result (F=1.17 → Pf 18%) — the exact technique the manual attributes to Duncan — and bracket the Slide2 per-method simulated Pf table. FOSM-ON-SURFACE is deferred: the slope geometry (surface + ocean level + Duncan's noncircular surface) is a manual figure lost in extraction, AND the module has no "su linear in depth with a single shared random gradient" law (its FOSM varies phi/c/cu/gamma independently, so thin undrained sub-layers would over-count the gradient variance). NOTE the Slide2 beta_LN & Pf columns are Latin-hypercube outputs that do NOT obey pf=Φ(−β); only Duncan's own quote is the closed-form anchor. |
