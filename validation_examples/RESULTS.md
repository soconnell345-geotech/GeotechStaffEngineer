...
| V-028 | slope_stability weak seam + piezo + surcharge, noncircular (ACADS 4 / Slide2 #9) | `weak_layer` search → degenerate Spencer 0.05-0.18; hand block Spencer 0.53 (water) / 0.61 (dry) | Spencer 0.760, GLE 0.720, Janbu-c 0.734; referee 0.78 | block −20 to −30% | **N/A (discrepancy)** | Not cleanly reproducible: (a) the module's `weak_layer` noncircular search returns DEGENERATE surfaces (~0.05-0.18) for this seam — a search-robustness pathology (B2: reject degenerate/short-span surfaces); (b) a hand block along the seam gives ~0.53-0.61 vs referee 0.76, sensitive to scarp construction and to the conservative full-head pore pressure (SS-3) on the inclined piezometric surface. Module also supports only ONE surcharge zone (problem has bench + crest — B2). Core LE methods validated by the 5 clean problems. No forced test. |
