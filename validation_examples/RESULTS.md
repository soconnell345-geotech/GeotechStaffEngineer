...
| V-037 | slope_stability rapid drawdown — USACE 2-stage (EM 1110-2-1902 / Slide2 #95) | FOS 1.228 (stage-1 full-pool 2.37) | Army-Corps 2-stage 1.347; referee 1.35 | −8.8% | **CONVENTION (approximate)** | New `rapid_drawdown_fos(method='corps_2stage')` closes the designed stub. Stage 1 (full-pool effective stresses via GLE) → stage 2 (combined R/effective undrained envelope) → drawdown-pool FOS. The ~9% gap is the RASTER-read upstream-face geometry + the FLAT-phreatic stage-1 pore pressure (vs Slide's steady-seepage flow net — a declined phreatic recovers 1.35). The undrained drawdown (1.23) is far below a drained drawdown of the same dam (2.11), as expected. |
| V-038 | slope_stability rapid drawdown — Duncan-Wright-Wong 3-stage (Slide2 #96) | FOS 1.279 | DWW 3-stage 1.443; referee 1.44 | −11.4% | **CONVENTION (approximate)** | Same dam, `method='duncan_3stage'`: Kc-interpolated undrained strength (between the Kc=1 R-envelope and the Kc=Kf drained envelope) + stage-3 drained substitution. Reproduces the published ORDERING — the 3-stage is LESS conservative than the 2-stage (1.279 > 1.228, as 1.443 > 1.347). Absolute gap = the V-037 geometry/seepage residual + the R-envelope total-vs-effective consolidation convention (not pinned by the published data). #98 (5-material Walter Bouldin Dam) not attempted (Fig 98.1 not reliably readable). Framework complete + tested (slope_stability/tests/test_rapid_drawdown.py). |
