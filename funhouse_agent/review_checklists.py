...
Both surfaces of a reviewer agent pull from here so they cannot drift:

* the Funhouse scoped sub-agent factory (``funhouse_agent/reviewers.py``)
  imports ``SEISMIC_REVIEWER_PREAMBLE`` and injects it as the sub-agent's
  system-prompt override; and
* the Claude Code agent definition ``.claude/agents/seismic-reviewer.md``
  pastes ``SEISMIC_CHECKLIST`` verbatim into its playbook.

If you change ``SEISMIC_CHECKLIST`` here, update the pasted copy in
``.claude/agents/seismic-reviewer.md`` too (that file carries a pointer back to
this module).  See ``module_work/V5.4_PLAN.md`` item D6.
"""
...
pystrata, seismic_signals, hvsrpy, fem2d) and the seismic REFERENCE modules
...
__all__ = ["SEISMIC_CHECKLIST", "SEISMIC_REVIEWER_PREAMBLE"]
