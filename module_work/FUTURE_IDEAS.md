...
## REPO SLIMMING + EXAMPLE-PRODUCT INVENTORY (2026-09-22; PARKED behind Tiny Apps)

**Why.** A DOS LLM reviewer said the package is "very large — millions of
tokens to consume, even without the geotech references". The owner asked
whether there is dead weight, thought `salib_agent` had been retired (it
had not), and ruled (2026-09-22): **keep `salib_agent` and `pystra_agent`;
the worked-example extracts stay on GitHub (public-domain government
documents, kept so future harnesses and models can test against them) but
never ship in the wheel; the example-product inventory needs a PROPER
REVIEW — on this list, behind the Tiny Apps work.**

**What was measured** (master `5bcea17` = v5.26.0; tokens ≈ bytes/4):

| What | Files | Size | ≈ tokens |
|---|---|---|---|
| Whole repo, tracked | 1,136 | 57 MB | 14.6M |
| `docs/` (generated HTML examples 11 MB, manual PDF 5 MB, `plotly.min.js` 4.7 MB, gallery) | 60 | 29 MB | 7.4M |
| `module_work/` (tracked; 5 MB is Mecklenburg DWGs) | 131 | 8.6 MB | 2.2M |
| `validation_examples/` (mostly `extracts/*.png`) | 62 | 3.8 MB | 1.0M |
| `sample_pdfs/` (generated by `generate_all_pdfs.py`) | 13 | 3.1 MB | 0.8M |
| **Python source, no tests** | ~500 | **8.0 MB** | **2.0M** |
| Python tests | ~360 | 4.1 MB | 1.1M |
| **The wheel (what pip installs)** | 456 | **7.1 MB** | **1.8M** |
| of which `report_ingest` (incl. 220 KB `cluster_scoring.py`) | 38 | 1.8 MB | 0.45M |
| of which `funhouse_agent` | 114 | 1.7 MB | 0.42M |
| `foundry/` + `foundry_test_harness/` (retired route; repo only) | 33 | 0.53 MB | 0.14M |

So the reviewer's number is the REPO, and 60 % of it is not code; the code
is ~2M tokens (two trains dominate: `report_ingest` and the agent layer)
and the wheel ~1.8M. **Already true today:** `validation_examples/`,
`module_work/`, `docs/`, `sample_pdfs/`, `foundry/` and
`foundry_test_harness/` are excluded from the wheel (`pyproject`
`packages.find.exclude`; 0 files from any of them in the 5.26.0 wheel).

**Dependency facts the work will need** (explored 2026-09-22):
- `foundry/`: no non-test module imports it. Seven test files keep their
  foundry tests in separable classes (`TestFoundryMetadata` /
  `TestFoundryIntegration` / `TestFoundryAgentMetadata`); the stragglers are
  two `test_foundry_agent_runs` methods in
  `liquepy_agent/tests/test_liquepy_agent.py` (~L455, ~L546) and a
  module-level `foundry_agent` fixture in
  `pystra_agent/tests/test_pystra_agent.py` (~L308). `foundry_test_harness`
  is in pyproject `testpaths`; CI runs bare `pytest -q`.
- Generated and read by nothing: `docs/examples/*.html`
  (`geo_project/demo_model_setup.py`), `docs/gallery/` (`build_gallery.py`
  — "delete figures/ and re-run"), `docs/user_manual/` and the manual PDF
  (`build_manual.py` → `render_pdf.py`), `sample_pdfs/`
  (`generate_all_pdfs.py`), `docs/geotech_eval_*.json`,
  `docs/geotech_test_suite_results.json`, root `suite_trial.json(.md)`.
- `module_work/` couplings a move must keep:
  `concrete_props_agent/tests/test_retired_library_oracle.py` reads
  `module_work/structural_native/oracles.json`;
  `funhouse_agent/deep/tests/test_triage_feedback_offline.py` loads
  `module_work/triage_feedback.py`; `module_work/report_ingest_harness/`
  is its own 229-test dev suite. planlens carries NONE of the Mecklenburg
  ground truth (its `test_readme_claims.py` hardcodes the numbers).
- `HANDOFF.md` (163 KB) and `CLAUDE.md` (148 KB): only
  `webapp/tests/test_docs_currency.py` reads them — the `## … CURRENT
  WORKING STATE` and `## 0a-current` headings with the version inside,
  `docs/DATABRICKS_INSTALL.md` + `HANDOFF.md` in CLAUDE.md's first 2,500
  characters, the install guide's version row. ~80 % of HANDOFF is inside
  the gated `0a-current` section; the `0a-prev*` history is ungated.
  **CLAUDE.md is loaded into every agent session (~37k tokens).**

**Backlog, ranked:**
1. **Example-product inventory review (owner + agent).** What
   `docs/examples`, `docs/gallery`, `sample_pdfs`, the `calc_package`
   templates and `worked_examples.json` are FOR (the idea was reference
   material for the calc-package and visualisation agents), which are
   outdated, which stay as regenerable builders only. Decide the home for
   generated artefacts (GitHub Release assets vs regenerate on demand)
   AFTER that review, not before.
2. Delete `foundry/` + `foundry_test_harness/`: excise the seven test
   classes, the two stray methods and the fixture; drop the testpaths entry
   and the `foundry*` exclude lines; CLAUDE.md inventory.
3. Drop the run-result JSONs (`docs/geotech_eval_*`,
   `geotech_test_suite_results.json`, `suite_trial.json*`) — git history
   keeps them; `module_work/triage_feedback.py`'s default path becomes an
   argument.
4. Trim `CLAUDE.md` to current state + triggers + architecture + inventory,
   moving the release-by-release narrative to `docs/RELEASE_HISTORY.md`;
   move HANDOFF `0a-prev*` sections to `docs/HANDOFF_HISTORY.md`. Keep the
   gated headings and the docs-currency test green.
5. `module_work/` home — decide with item 1 (a private `geotech-workbench`
   repo vs stay); keep the two test-read files and the Mecklenburg truth
   reachable either way.
6. `validation_examples/extracts/`: STAYS (owner ruling); already not in
   the wheel. Consider a `README` line saying so.
7. Not dead weight, only big: `report_ingest/cluster_scoring.py` and the
   eval harness ship because the owner runs them on the cluster.
