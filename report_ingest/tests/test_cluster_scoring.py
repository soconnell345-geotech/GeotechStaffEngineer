...
def cluster(tmp_path):
    """A corpus folder and an out_dir with every report already run."""
    ...
    for rid in ("R36", "R31"):
        (corpus_dir / f"{rid}.pdf").write_bytes(b"%PDF-1.7\n")
...
def test_a_report_with_a_run_file_is_skipped_so_a_resume_is_free(cluster,
                                                                 capsys):
    corpus_dir, out, oos = cluster
    cs.score_on_cluster(corpus_dir=corpus_dir, out_dir=out,
...
def test_the_set_is_scored_before_and_after_from_the_saved_runs(cluster):
    corpus_dir, out, oos = cluster
    results = cs.score_on_cluster(corpus_dir=corpus_dir, out_dir=out,
                                  prompter=object(), oos_labels=oos,
                                  sets=("oos_blind",))
